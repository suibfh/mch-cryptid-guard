(() => {
  ...
  function updateHeroDetail() {
    ...
    const initialId = (h.initialUpgrades && h.initialUpgrades[0]) || null;
    const initialExt = initialId ? upgrades[initialId] : null;
    const skillText = h.skill ? h.skill.replace(/^初期エクステ:\s*/, "") : "初期エクステなし";
    ui.heroDetail.innerHTML = `
      ...
      <div class="hero-skill">
        ${initialExt ? `<img class="hero-skill-icon" src="${initialExt.asset}" alt="${initialExt.name}">` : ""}
        <div><b>初期エクステ</b><span>${skillText}</span></div>
      </div>
    `;
  }
